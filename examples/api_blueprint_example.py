...
class CustomerDataSchema(Schema):
    ...
    country = fields.Str(
        required=True,
        validate=lambda x: x.upper() in ["US", "GB", "DE", "PH", "IE"],
    )
...
class CreateOrderSchema(Schema):
    ...
    carrier = fields.Str(
        required=False,
        validate=lambda x: x.upper() in ["FEDEX", "UPS", "DHL", "USPS"],
    )
...
def log_request(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        try:
            ...
            current_app.logger.info(
                f"API Response: {request.path} completed in {duration:.3f}s"
            )
...
def create_order():
    ...
    try:
        ...
        return (
            jsonify({"error": "Order validation failed", "details": str(e)}),
            400,
        )
    except Exception as e:
...
def handle_validation_error(error):
    ...
    return (
        jsonify({"error": "Validation failed", "details": error.messages}),
        400,
    )
...
def _validate_api_key(api_key):
    ...
    return (
        api_key in valid_keys or api_key == "development-key"
    )  # Remove dev key in production
