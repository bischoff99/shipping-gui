...
class EasyshipAPI:
    ...
    def make_request(
        ...
        """Make API request to Easyship with comprehensive error handling"""
        try:
            ...
            if response.status_code in [200, 201]:
                try:
                    ...
                    print(f"❌ Easyship JSON Parse Error: {json_error}")
                    return None
            else:
                print(
                    f"❌ Easyship API Unexpected Status: {
            ...
            print(
                f"❌ Easyship API Timeout: Request to {endpoint} timed out after {timeout}s"
            )
            return None
        except requests.exceptions.ConnectionError:
            print(
                f"❌ Easyship API Connection Error: Unable to connect to {
                    self.base_url}"
            )
            return None
        except requests.exceptions.HTTPError as http_error:
            print(f"❌ Easyship API HTTP Error: {http_error}")
            return None
        except requests.exceptions.RequestException as req_error:
            print(f"❌ Easyship API Request Error: {req_error}")
            return None
        except Exception as e:
            print(f"❌ Easyship Unexpected Error: {e}")
            return None
    ...
    def create_shipment(
        self, customer_data: Dict, products: List[Dict], origin_address_id: str
    ) -> Optional[Dict]:
        ...
        shipment_data = {
            ...
            "destination_address": {
                ...
                "state": (
                    customer_data.get("state", "") if country_code == "US" else None
                ),
            },
            "parcels": [
                {
                    "total_actual_weight": sum(
                        float(p.get("weight", 0.5)) for p in products
                    ),
                    "box": {"length": 30, "width": 25, "height": 15},
                    "items": [
                        {
        ...
        shipment_data["destination_address"] = {
            k: v
            for k, v in shipment_data["destination_address"].items()
            if v is not None
        }
    ...
    def book_fedex_rate(self, shipment_id: str) -> Optional[Dict]:
        ...
        if fedex_rate:
            ...
            return self.make_request(
                f"/shipments/{shipment_id}/buy", "PATCH", booking_data
            )
