    ...
    def check_newman():
        ...
        try:
            result = subprocess.run(
                ["newman", "--version"], capture_output=True, text=True, timeout=10
            )
...
class NewmanTestRunner:
    ...
    def run_tests(self):
        ...
        try:
            ...
            print_colored(
                "❌ Newman not found. Install with: npm install -g newman", Colors.RED
            )
            return False
        except Exception as e:
...
def main():
    ...
    if success:
        ...
        print_colored(
            "❌ Some tests failed or encountered errors", Colors.RED + Colors.BOLD
        )
        sys.exit(1)
