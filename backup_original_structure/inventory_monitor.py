...
class RealTimeInventoryMonitor:
    ...
    def start_monitoring(self, check_interval: int = 60):
        ...
        thread.start()
        self.logger.info(
            f"Inventory monitoring started with {check_interval}s interval"
        )
                ...
                for sellable in sellables:
                    ...
                    for entry in stock_entries:
                        ...
                        if available == 0:
                            ...
                            severity = (
                                "high" if available <= threshold * 0.5 else "medium"
                            )
                            self._create_alert(
                                sku,
                                product_name,
                            ...
                            self._create_alert(
                                ...
                                warehouse_name,
                                available,
                                threshold * self.default_overstock_multiplier,
                                "overstock",
                                "low",
                            )
    ...
    def _create_alert(
        ...
        self.logger.warning(
            f"New {severity} alert: {alert_type} for {sku} at {warehouse_name}"
        )
    ...
    def record_movement(
        ...
        self.logger.info(
            f"Movement recorded: {movement_type} {quantity} units of {product_sku}"
        )

    def get_movements(
        self, product_sku: str = None, warehouse_id: str = None, hours: int = 24
        ...
        try:
            ...
            for product in products:
                ...
                if product_total_stock == 0:
                    out_of_stock += 1
                elif product_total_stock <= self.thresholds.get(
                    product.get("sku", ""), self.default_low_stock
                ):
                    low_stock += 1
        ...
        try:
            ...
            for product in products:
                ...
                if total_stock <= threshold:
                    ...
                    # Simple logic: order enough for 30 days based on average
                    # daily usage
                    suggested_quantity = max(
                        threshold * 2, 50
                    )  # At least double threshold or 50 units
                    ...
                    suggestions.append(
                        {
                            ...
                            "estimated_cost": float(product.get("cost_price", 0))
                            * suggested_quantity,
                        }
                    )
            ...
            self.logger.error(
                f"Error generating reorder suggestions: {
                    str(e)}"
            )
            return []
    ...
    def export_inventory_report(self, format: str = "json") -> str:
        ...
        try:
            report_data = {
                ...
                "recent_movements": [
                    asdict(movement) for movement in self.get_movements()
                ],
