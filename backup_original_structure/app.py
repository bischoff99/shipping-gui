...
# Create sample data if database is empty (for development)
...
inventory_monitor = RealTimeInventoryMonitor()
...
def api_get_routing():
    """API endpoint to get routing decision for customer data"""
    try:
        data = request.get_json()
        customer_data = data.get("customer_data", {})
        selected_carrier = data.get("carrier", "")

        if not customer_data:
            return (
                jsonify({"status": "error", "message": "No customer data provided"}),
                400,
            )

        # Get warehouses based on selected carrier or default
        warehouses, _ = get_warehouses_and_products(selected_carrier)
        ...
        return jsonify(
            {
                ...
                "routing": {
                    ...
                    "warehouse": (
                        routing_decision.warehouse_info.get("name", "Unknown")
                        if routing_decision.warehouse_info
                        else "Unknown"
                    ),
                    "warehouse_region": (
                        routing_decision.warehouse_info.get("region", "Unknown")
                        if routing_decision.warehouse_info
                        else "Unknown"
                    ),
