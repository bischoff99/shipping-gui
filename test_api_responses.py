...
from validation import validate_customer_data, validate_order_data, ValidationResult
...
class FlaskAPIEndpointsTest(APITestCase):
    ...
    def test_sync_data_success(self, mock_easyship_products, mock_veeqo_products, 
                              mock_easyship_addresses, mock_veeqo_warehouses):
...
class ExternalAPITest(APITestCase):
    ...
    def test_easyship_create_shipment_success(self, mock_post):
        ...
        mock_response = Mock()
        mock_response.status_code = 201
        mock_response.json.return_value = {'id': 'ship123', 'status': 'created'}
        mock_post.return_value = mock_response
...
class ErrorHandlingTest(APITestCase):
    ...
    def test_api_endpoints_with_exceptions(self):
        ...
        with patch('app.inventory_monitor.get_active_alerts', side_effect=Exception("Database error")):
            response = self.client.get('/api/inventory_alerts')
            data = self.assertJSONResponse(response, 500)
            self.assertEqual(data['status'], 'error')
            self.assertIn('message', data)
    ...
    def test_auto_sync_start_with_custom_interval(self, mock_start):
        ...
        response = self.client.post('/api/start_auto_sync',
                                  json={'interval': 10},
                                  content_type='application/json')
...
class ResponseFormatTest(APITestCase):
    ...
    def test_error_response_format(self):
        ...
        with patch('app.inventory_monitor.get_inventory_summary', side_effect=Exception("Error")):
            response = self.client.get('/api/inventory_summary')
            data = self.assertJSONResponse(response, 500)
            self.assertEqual(data['status'], 'error')
            self.assertIn('message', data)
