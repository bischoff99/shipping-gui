...
class AdvancedProductSync:
    ...
    async def sync_products_bidirectional(self) -> Dict[str, Any]:
        ...
        try:
            ...
            duration = self.sync_stats.sync_duration
        ...
        for product in products:
            try:
                ...
                if existing:
                    ...
                    product_data = self._product_to_veeqo_format(product)
                    self.veeqo_api.update_product(existing["id"], product_data)
                    results["updated"] += 1
                else:
                    # Create new product
                    product_data = self._product_to_veeqo_format(product)
                    self.veeqo_api.create_product(product_data)
                    results["created"] += 1

                product.sync_status = "synced"

            except Exception as e:
                error_msg = f"Error syncing product {
                    product.sku} to Veeqo: {
                    str(e)}"
    ...
    def start_auto_sync(self, interval_minutes: int = 5):
        ...
        msg = f"Auto-sync started with {interval_minutes} minute interval"
    ...
    def get_product_performance(self) -> Dict[str, Any]:
        ...
        total_value = sum(p.price * sum(p.inventory.values()) for p in products)
