...
class AdvancedProductSync:
    ...
    def _merge_products(self, veeqo_products: List[Product], easyship_products: List[Product]) -> List[Product]:
    ...
    async def sync_products_bidirectional(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.sync_stats.synced_products = len([p for p in all_products if p.sync_status == 'synced'])
            self.sync_stats.pending_products = len([p for p in all_products if p.sync_status == 'pending'])
            self.sync_stats.error_products = len([p for p in all_products if p.sync_status == 'error'])
            
            self.logger.info(f"Sync completed in {self.sync_stats.sync_duration:.2f} seconds")
        ...
        for product in products:
            try:
                ...
                if existing:
                    ...
                    self.veeqo_api.update_product(existing['id'], self._product_to_veeqo_format(product))
                    results['updated'] += 1
                else:
                    # Create new product
                    self.veeqo_api.create_product(self._product_to_veeqo_format(product))
                    results['created'] += 1
                
                product.sync_status = 'synced'
                
            except Exception as e:
                self.logger.error(f"Error syncing product {product.sku} to Veeqo: {str(e)}")
    ...
    def get_product_performance(self) -> Dict[str, Any]:
        ...
        total_value = sum(p.price * sum(p.inventory.values()) for p in self.products_cache.values())
        avg_price = sum(p.price for p in self.products_cache.values()) / len(self.products_cache)
    ...
    def _get_top_products_by_value(self, limit: int = 10) -> List[Dict]:
        ...
        return sorted(products_with_value, key=lambda x: x['total_value'], reverse=True)[:limit]
