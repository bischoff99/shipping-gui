...
class IntegrationTestConfig:
    ...
    RUN_REAL_API_TESTS = os.environ.get('RUN_INTEGRATION_TESTS', 'False').lower() == 'true'
    
    # Rate limiting settings
    API_CALL_DELAY = float(os.environ.get('API_CALL_DELAY', '1.0'))  # seconds between calls
...
class APIIntegrationTestCase(unittest.TestCase):
    ...
    def make_rate_limited_call(self, api_call):
        ...
        if self.api_call_count >= self.config.MAX_API_CALLS_PER_TEST:
            self.skipTest(f"Maximum API calls ({self.config.MAX_API_CALLS_PER_TEST}) reached for this test")
    ...
    def skipIfNoRealAPITests(self):
        ...
        if not self.config.RUN_REAL_API_TESTS:
            self.skipTest("Real API testing disabled. Set RUN_INTEGRATION_TESTS=true to enable")
...
class VeeqoAPIIntegrationTest(APIIntegrationTestCase):
    ...
    def test_veeqo_api_connection(self):
        ...
        print(f"✅ Veeqo API connection successful. Found {len(warehouses)} warehouses")
        
        if warehouses:
            warehouse = warehouses[0]
            required_fields = ['id', 'name']
            for field in required_fields:
                self.assertIn(field, warehouse, f"Warehouse missing required field: {field}")
    ...
    def test_veeqo_random_products(self):
        ...
        products = self.make_rate_limited_call(lambda: self.veeqo_api.get_random_products(3))
...
class EasyshipAPIIntegrationTest(APIIntegrationTestCase):
    ...
    def test_easyship_api_connection(self):
        ...
        print(f"✅ Easyship API connection successful. Found {len(addresses)} addresses")
        
        if addresses:
            address = addresses[0]
            required_fields = ['id']
            for field in required_fields:
                self.assertIn(field, address, f"Address missing required field: {field}")
    ...
    def test_easyship_random_products(self):
        ...
        products = self.make_rate_limited_call(lambda: self.easyship_api.get_random_products(3))
        ...
        for i, product in enumerate(products):
            ...
            print(f"  Product {i+1}: {product['title']} - ${product['price']} ({product['weight']}kg)")
...
class EndToEndIntegrationTest(APIIntegrationTestCase):
    ...
    def test_complete_order_flow_veeqo(self):
        ...
        # Step 2: Get products
        products = self.make_rate_limited_call(lambda: self.veeqo_api.get_random_products(2))
    ...
    def test_complete_order_flow_easyship(self):
        ...
        # Step 2: Get products
        products = self.make_rate_limited_call(lambda: self.easyship_api.get_random_products(2))
        self.assertEqual(len(products), 2)
        ...
        print(f"  Selected address: {selected_address.get('state', 'Unknown')}")
        print(f"  Selected products: {[p['title'] for p in products]}")
    ...
    def test_api_endpoint_integration(self):
        ...
        self.assertIn(response.status_code, [200, 500])  # May fail due to API limits
...
def print_integration_test_info():
    ...
    print("=" * 60)
    print(f"Real API Testing: {'ENABLED' if IntegrationTestConfig.RUN_REAL_API_TESTS else 'DISABLED'}")
    print(f"API Call Delay: {IntegrationTestConfig.API_CALL_DELAY}s")
    print(f"Max API Calls per Test: {IntegrationTestConfig.MAX_API_CALLS_PER_TEST}")
    ...
    print(f"  VEEQO_API_KEY: {'SET' if os.environ.get('VEEQO_API_KEY') else 'NOT SET'}")
    print(f"  EASYSHIP_API_KEY: {'SET' if os.environ.get('EASYSHIP_API_KEY') else 'NOT SET'}")
    print("=" * 60)
...
if __name__ == '__main__':
    ...
    print(f"Skipped: {result.testsRun - len(result.failures) - len(result.errors) - len([t for t in result.skipped if hasattr(result, 'skipped')])}")
