...
Handle UPS and DHL orders that should be routed through Veeqo
...
class VeeqoOrderProcessor:
    """Processor for Veeqo orders, including customer and order management."""
    ...
    def __init__(self):
        ...
        # Predefined Veeqo customer data (UPS and DHL orders
        # based on state preferences)
    ...
    def get_veeqo_customer_by_name(self, name: str) -> Optional[Dict]:
        """Get Veeqo customer data by name"""
    ...
    def get_customer_by_name(self, name: str) -> Optional[Dict]:
        """Stub for test compatibility: returns a customer dict by name if found."""
        for customer in self.veeqo_customers:
            if customer.get("name") == name:
                return customer
        return None
    ...
    def create_veeqo_order(
        self, customer_data: Dict, warehouse_id: Optional[int] = None
    ) -> Optional[Dict]:
        ...
        try:
            ...
            if not warehouse_id:
                ...
                if warehouse:
                    ...
                    self.logger.error(
                        "No suitable warehouse found for %s",
            ...
            if not products:
                self.logger.warning(
                    "No products available, using fallback products for %s",
                    customer_data.get("name"),
                )
                products = [
                    {
            ...
            normalize_customer_data(customer_data)
            results = []
            self.logger.info("Processing %d Veeqo orders...", len(self.veeqo_customers))
            for customer in self.veeqo_customers:
                self.logger.info(
                    "Processing %s order for %s (State pref: %s)...",
                    customer["carrier"],
                    customer["name"],
            ...
            self.logger.info(
                "Processed %d/%d Veeqo orders successfully",
                successful,
                len(results),
            )
            return results
        except Exception as e:
            self.logger.error("Unexpected error in create_veeqo_order: %s", e)
            return None
